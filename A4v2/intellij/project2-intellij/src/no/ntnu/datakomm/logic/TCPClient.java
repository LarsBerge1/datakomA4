...
public class TCPClient implements ChatClientFacade {
    ...
    public boolean sendPublicMessage(String message) {
        ...
        return sendTextMessage("msg","",message);
    }
    ...
    private void parseIncomingCommands() {
        while (isConnectionActive()) {
            ...
            if (line != null && line.length() > 0) {
                ...
                {
                    onLoginResult(true,params);
                }
                if (cmd.equals("loginerr"))
                {
                ...
                if (cmd.equals("msg")) {
                    ...
                    onMsgReceived(false,split[0],split[1]);
                }
                if (cmd.equals("privmsg"))
                {
                    String[] split = params.split(" ",2);
                    onMsgReceived(true,split[0],split[1]);
                }
                if (cmd.equals("cmderr"))
                {
                    onCmdError(params);
                }
                if (cmd.equals("msgerr"))
                {
                    onMsgError(params);
                }
